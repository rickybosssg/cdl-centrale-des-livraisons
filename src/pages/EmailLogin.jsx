...
function Field({ icon: Icon, type = "text", placeholder, value, onChange, onKeyDown, autoComplete, inputMode, extra }) {
  return (
    <div style={{ position: "relative", marginBottom: "12px" }}>
      <Icon size={16} style={{ position: "absolute", left: "14px", top: "50%", transform: "translateY(-50%)", color: "#9CA3AF", pointerEvents: "none" }} />
      <input
        type={type}
        ...
        inputMode={inputMode}
        style={{ width: "100%", height: "50px", border: "2px solid #E5E7EB", borderRadius: "14px", paddingLeft: "42px", paddingRight: extra ? "44px" : "14px", fontSize: "15px", color: "#111", outline: "none", boxSizing: "border-box", background: "#F9FAFB" }}
...
export default function EmailLogin() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (ok && token) {
        ...
        setMessage(status === 401 || status === 400 ? "Email ou mot de passe incorrect" : "Une erreur est survenue. Réessayez.");
      }
    } catch {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (ok && token) {
        ...
        try { localStorage.setItem('cdl_admin_pwd', password); } catch(_) {}
        base44.auth.updateMe({ full_name: fullName, telephone: telephone.trim() }).catch(() => {});
        ...
        setMessage(
          ...
          isNetwork ? "Une erreur est survenue. Réessayez dans un instant." :
      ...
      setMessage(isOffline ? "Connexion internet indisponible — vérifiez votre réseau" : "Une erreur est survenue. Réessayez.");
      setLoading(false);
    }
  };
  ...
  const handleForgot = async () => {
    ...
    try {
      ...
      goTo("login");
    } catch { setMessage("Erreur réseau — vérifiez votre connexion et réessayez"); }
  ...
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg, ${DARK} 0%, ${BLUE} 60%, #4A90E2 100%)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 16px", fontFamily: "Inter, system-ui, sans-serif", overflowY: "auto" }}>

      {/* Logo */}
      <div style={{ textAlign: "center", marginBottom: "24px" }}>
        <div style={{ width: "72px", height: "72px", borderRadius: "20px", background: "rgba(255,255,255,0.15)", border: "1.5px solid rgba(255,255,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 10px", backdropFilter: "blur(10px)", overflow: "hidden" }}>
          <img src="https://media.base44.com/images/public/69c3c74fc4b62396dca61751/1eb51398f_Screenshot_20260330_132434_WhatsApp.jpg" alt="CDL" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "18px" }} onError={e => { e.target.style.display = "none"; }} />
        </div>
        <h1 style={{ fontSize: "28px", fontWeight: "900", color: "white", letterSpacing: "4px", margin: 0 }}>CDL</h1>
        <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.6)", margin: "4px 0 0", fontWeight: "500" }}>Centrale des Livraisons</p>
      </div>

      {/* Card */}
      <div style={{ background: "white", padding: "28px 22px 24px", borderRadius: "28px", width: "100%", maxWidth: "390px", boxShadow: "0 32px 80px rgba(0,0,0,0.35)" }}>
        <h2 style={{ fontSize: "20px", fontWeight: "800", color: "#111", margin: "0 0 2px" }}>
          {mode === "login" ? "Connexion" : mode === "register" ? "Créer un compte" : "Mot de passe oublié"}
        </h2>
        <p style={{ fontSize: "13px", color: "#6B7280", margin: "0 0 18px" }}>
          {mode === "login" ? "Accédez à votre espace CDL" : mode === "register" ? "Rejoignez la Centrale des Livraisons" : "Un lien vous sera envoyé par email"}
        </p>

        {/* Succès */}
        {successMsg && (
          <div style={{ background: "#f0fdf4", border: "1.5px solid #86efac", borderRadius: "14px", padding: "12px 14px", fontSize: "13px", color: "#15803d", marginBottom: "14px", lineHeight: "1.5" }}>
            {successMsg}
          </div>
        )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "12px" }}>
            <div style={{ position: "relative" }}>
              ...
              <input
                ...
                onChange={e => { setPrenom(e.target.value); clear(); }}
                autoComplete="given-name"
                style={{ width: "100%", height: "48px", border: "2px solid #E5E7EB", borderRadius: "14px", paddingLeft: "36px", paddingRight: "10px", fontSize: "14px", color: "#111", outline: "none", boxSizing: "border-box", background: "#F9FAFB" }}
                onFocus={e => { e.target.style.borderColor = BLUE; e.target.style.background = "white"; }}
                onBlur={e => { e.target.style.borderColor = "#E5E7EB"; e.target.style.background = "#F9FAFB"; }}
              />
            </div>
            <div style={{ position: "relative" }}>
              ...
              <input
                ...
                onChange={e => { setNom(e.target.value); clear(); }}
        ...
        {mode === "register" && (
          <div style={{ position: "relative", marginBottom: "12px" }}>
            ...
            <input
              type="tel" placeholder="Numéro de téléphone (ex: 70000000)" value={telephone}
              onChange={e => { setTelephone(e.target.value); clear(); }}
        ...
        {/* Erreur */}
        {message && (
          <p style={{ color: "#EF4444", fontSize: "13px", marginBottom: "12px", fontWeight: "500", lineHeight: "1.4", padding: "10px 12px", background: "#FEF2F2", borderRadius: "10px", border: "1px solid #FECACA" }}>
            {message}
          </p>
        )}
        ...
        >
          {loading
            ? <><Loader2 style={{ animation: "spin 1s linear infinite", width: 18, height: 18 }} /> Chargement...</>
        ...
        {mode === "login" && (
          <button
            ...
            style={{ background: "none", border: "none", color: BLUE, fontSize: "13px", fontWeight: "600", cursor: "pointer", marginBottom: "12px", display: "block", width: "100%", textAlign: "center" }}
          >
            Mot de passe oublié ?
          </button>
        )}
        ...
        {mode === "login" ? (
          <div style={{ background: "linear-gradient(135deg, #16a34a 0%, #15803d 100%)", borderRadius: "18px", padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", boxShadow: "0 4px 14px rgba(22,163,74,0.25)" }}>
            <div>
            ...
            <button
              ...
              style={{ background: "white", border: "none", color: "#15803d", fontSize: "13px", fontWeight: "800", cursor: "pointer", padding: "9px 18px", borderRadius: "12px", whiteSpace: "nowrap", boxShadow: "0 2px 8px rgba(0,0,0,0.12)" }}
            >
              Créer →
            </button>
          </div>
          ...
          <button
            onClick={() => goTo("login")}
            style={{ background: "none", border: "none", color: BLUE, fontSize: "14px", fontWeight: "700", cursor: "pointer", width: "100%", textAlign: "center", textDecoration: "underline" }}
          >
      ...
      <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.45)", marginTop: "20px", textAlign: "center" }}>
        © 2025 CDL — Centrale des Livraisons, Ouagadougou
      </p>
    </div>
  );
}