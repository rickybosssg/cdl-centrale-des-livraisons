...
import { Search, Store, Package, ShoppingBag, MapPin, Trash2 } from "lucide-react";
    ...
    <div className="space-y-3">
      ...
      {!loading && filtered.length === 0 && (
        <div className="text-center py-12 space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Aucune commande Mall pour le moment</p>
        </div>
      )}
      {!loading && (
              ...
              <CardContent className="p-3 space-y-2">
                ...
                {cmd.course_id && (
                  <div className="text-[10px] text-blue-600 font-medium">
                    🛵 Course CDL · ID: {String(cmd.course_id).slice(0, 8)}…
                  </div>
                )}
              </CardContent>
            </Card>
...
function MallBoutiques({ isAdmin }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {!loading && filtered.length === 0 && (
        <div className="text-center py-12 text-sm text-muted-foreground">Aucune boutique disponible</div>
      )}
      {!loading && (
              ...
              <CardContent className="p-2 space-y-1">
                ...
                {isAdmin && (
                  <div className="flex gap-1 pt-1">
                    <button
                      onClick={() => navigate(`/commerce/${p.id}`)}
                      className="flex-1 text-[10px] py-1 rounded border text-center hover:bg-muted"
                    >
                      Voir
                    </button>
                    <button
...
function AdminProduits() {
  ...
  return (
    <div className="space-y-3">
      ...
      {!loading && filtered.length === 0 && (
        <div className="text-center py-12 text-sm text-muted-foreground">Aucun produit disponible</div>
      )}
      {!loading && (
        <div className="space-y-2">
          {filtered.map((prod) => (
            <Card key={prod.id}>
              <CardContent className="p-3 flex items-center gap-3">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  ...
                  <button
                    onClick={() => supprimerProduit(prod)}
                    className="text-[10px] px-2 py-1 rounded-full bg-red-50 text-red-600"
                  >
...
function AdminCommandes() {
  ...
  return (
    <div className="space-y-3">
      ...
      {!loading && filtered.length === 0 && (
        <div className="text-center py-12 text-sm text-muted-foreground">Aucune commande pour le moment</div>
      )}
      {!loading && (
        <div className="space-y-2">
          {filtered.map((cmd) => (
    ...
    const load = async () => {
      try {
        ...
        setStats({
          ...
          produits: safeProd.length,
          commandes: safeC.length,
