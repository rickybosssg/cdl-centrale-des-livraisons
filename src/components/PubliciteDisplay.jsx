...
export default function PubliciteDisplay({ userRole, userId, userEmail }) {
    ...
    const loadPublicites = async () => {
      try {
        ...
        const filtered = (allPubs || []).filter(pub => {
          ...
          if (userRole === 'livreur' && placement === 'home_livreur') return true;
          if (userRole === 'client' && placement === 'home_client') return true;
          if (userRole === 'partenaire' && placement === 'home_partenaire') return true;

          // Aussi check le champ destinataires pour compatibilité
          if (!destinataires || destinataires === 'tous') return true;
          ...
          return destinataires.includes(rolesMap[userRole]);
        });
  ...
  return (
    <div className="sticky top-0 z-40 w-full bg-white border-b border-primary/10 shadow-sm">
      <div className="max-w-6xl mx-auto p-3 relative">
        {/* Bannière */}
