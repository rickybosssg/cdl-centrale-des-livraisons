...
import { Upload, ArrowLeft, Store, Pill, ShoppingBag, UtensilsCrossed, Wine } from "lucide-react";
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-background">
        <div className="w-full max-w-sm space-y-6">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">Choisissez votre catégorie</p>
          </div>
    ...
    <div className="min-h-screen p-4 bg-background">
      <div className="max-w-sm mx-auto space-y-5 pb-8">
        <div className="text-center pt-4">
          <h2 className="text-xl font-bold">Votre commerce</h2>
          <p className="text-sm text-muted-foreground">{typeCommerce}</p>
        </div>

        <div className="space-y-3">
          <div><Label>Nom du commerce *</Label>
            <Input placeholder="Ex: Restaurant Le Délice" value={form.nom_commerce} onChange={e => setForm(f => ({ ...f, nom_commerce: e.target.value }))} /></div>
          ...
          <div><Label>Horaires d'ouverture</Label>
            <Input placeholder="Ex: Lun-Sam 7h-20h" value={form.horaires} onChange={e => setForm(f => ({ ...f, horaires: e.target.value }))} /></div>
          <div>
            <Label>Photo du commerce</Label>
            <div className="mt-1">
